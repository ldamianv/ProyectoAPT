  ...
  async function loadFefoData() {
    try {
      ...
      locationsData = data.locations.map(row => {
        // Transformar fechaProduccion a formato mes-año (por ejemplo, nov-2024)
        if (row.fechaProduccion) {
          ...
          // Si ya está en mes-año (por ejemplo, nov-2024), lo dejamos como está
    ...
    filteredData.forEach((row, index) => {
      ...
      html += `
        <tr>
          ...
          <td><input type="text" value="${row.fechaProduccion || ''}" placeholder="mes-año (ej. nov-2024)" pattern="^[a-z]{3}-\\d{4}$" onchange="updateField(${globalIndex}, 'fechaProduccion', this.value.toLowerCase())"></td>
